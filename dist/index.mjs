...
// src/namedays/index.ts
var namedays = [...namedaysDK, ...namedaysNO, ...namedaysSE];

// src/lib/filterByCountryCode.ts
function filterByCountryCode(countryCode) {
  if (!countryCode)
    return namedays;
...
// src/on.ts
function on(date, settings) {
  const filteredNamedays = filterByCountryCode(settings == null ? void 0 : settings.countryCode);
...
function today(settings) {
  ...
  const filteredNamedays = filterByCountryCode(settings == null ? void 0 : settings.countryCode);
  return filteredNamedays.filter(
    (nameday) => nameday.date.month === today2.month && nameday.date.day === today2.day
  );
}

// src/when.ts
function when(name, settings) {
  const filteredNamedays = filterByCountryCode(settings == null ? void 0 : settings.countryCode);
  return filteredNamedays.filter(
